	...
	**/
	...
	public function email($send_to ='' , $send_from ='' , $subject ='' ,  $msg ='') {
		...
		if(1 == 1)
		{
			...
			$config['smtp_user'] = 'info.ceucentral@gmail.com';
			...
			*/
			...
			$this->email->bcc('m.fazal@manageglobally.io');
    ...
    public function get_list_data($field)
    {
    	$list = $this->get_fields($field)['list_data'];
    	return array_filled($list) ? $list : array();
    }
	

	public function soft_delete($pkid)
	{
		$update_param = array();
        $update_param[$this->_status_field] = STATUS_DELETE;
        return $this->update_by_pk($pkid,$update_param);
	}


	public function get_associate_data($factory_id)
	{
		$param = array();
        $param['where_in'][$this->_associate_id] = array(0,$factory_id);
        $param['order'] = $this->_pk . " DESC";
        $param['where_string'] = $this->_status_field. ' < 2';
        return $this->find_all($param);
        //return $this->find_all_active($param);
	}


	// For Front All data show except Deleted Records
	public function get_all_data($param = array())
    {
        $status_fld = $this->get_table_status_field();
