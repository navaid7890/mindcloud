...
/* Hamburger Menu */
.menu-Bar { width: 30px; height: 27px; cursor: pointer; position: absolute; right: 15px; top: 0; bottom: 0px; margin: auto; z-index: 22; display: none; }
.menu-Bar span { display: block; height: 3px; width: 100%; background: #1d8fd9; position: absolute; transition: .6s all; border-radius: 100px; }
...
.coutArea{display: flex;}
/* Menu Css */
.cart-icon > i { color: #1d90d9; }
.menu { font-size: 0px; text-align: center; display: flex; align-items: center; justify-content:space-between; }
.menu li {margin-right: 16px;}
.menu li.last { margin: 0; }
.menu li a { text-transform: capitalize; display: block; font-size: 14px; color: #33415C; font-weight: 300; padding: 30px 0; position: relative;}
.menu li:nth-last-child(-n+2) a { display: none; }
.menu li:hover a,
.menu li.active a { text-shadow: 0 0 .65px #333, 0 0 .65px #333; }
.menu li a.active { text-shadow: 0 0 0.65px #333, 0 0 0.65px #333 !important;}

/* Drop Down Css */
.dropdown-nav { position: relative; }
.dropdownWrap { position: absolute; left: -20px; top: 30px; display: none; }
ul.dropdown { width: 257px; border-radius: 8px; background-color: #ffffff; z-index: 1; padding:0; text-align: left; box-shadow: 0 0 5px rgba(0, 0, 0, 0.1); }
ul.dropdown li { width: 100%; display: block; margin-bottom: 5px; }
ul.dropdown li a { font-size: 15px; letter-spacing: -0.1px; color: #636e72; padding: 12px 12px 12px 40px; position: relative; border-radius: 6px; line-height: normal; margin: 0; display: flex; align-items: center; }
...
li.dropdown-nav a:after{ content: "\f107"; display: inline-block; font-family: "Font Awesome 5"; font-size: 18px; font-weight: 300; font-stretch: normal; font-style: normal; letter-spacing: normal; text-align: center; color: #2d3436; -webkit-transition: all 0.4s ease-in-out; -moz-transition: all 0.4s ease-in-out; -o-transition: all 0.4s ease-in-out; transition: all 0.4s ease-in-out; margin-left: 10px; display: inline-flex; justify-content: flex-end; vertical-align: middle; align-items: center;}

li.dropdown-nav:hover ul.dropdownWrap { display: block;}
.menu .dropdownWrap li:nth-last-child(-n+2) a{ display: block;}
ul.dropdown li a:after{ display: none;}
