      ...
      <table id="table_id" class="table">
        ...
        <tbody>
         ...
         foreach ($enrolled_courses as $key => $value): ?>
           <tr>
             <td><?=$sno?></td>
             <td><img src="<?=get_image($value['course_image'],$value['course_image_path'])?>" style="height: 120px;"></td>
             <td><?=$value['item_product_name']?></td>
             <td><?=count($this->model_course->get_lectures($value['item_product_id']))?></td>
             <td><?=$value['course_identity']?></td>
