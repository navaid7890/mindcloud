...
<form class="cmxform horizontal-form tasi-form" 
	id="uploadCmsimage" 
	method="POST" 
	action="<?=$config['base_url']?>admin/course/upload_image" 
>
		<div class="form-body">
        	...
        	<input type = "hidden" value="<?=$form_data['course']['course_id']?>" name = "course[course_id]" />
...
$max_images = 4;
    ...
    {?>
         ...
         <?php if ($form_data['course']['course_is_image'.$i]  == 1): ?>
        ...
        <div class="col-md-4">
            ...
            <?php
            if(!empty($form_data['course']['course_image2'.$i])){
            ?>
                <img alt="" src="<?=g('base_url')?>assets/uploads/course/<?=$form_data['course']['course_image2'.$i]?>">
            <?php
            }
            else{
            ?>
...
<script>
	$(document).ready(function() {
		...
		$("#saveImageCms").click(function(){
	        ...
	        var url = "<?=$config['base_url']?>admin/course/upload_images/";
