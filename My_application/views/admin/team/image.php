...
<form class="cmxform horizontal-form tasi-form" 
	id="uploadCmsimage" 
	method="POST" 
	action="<?=$config['base_url']?>admin/team/upload_images" 
>
		<div class="form-body">
        	...
        	<input type = "hidden" value="<?=$form_data['team']['team_id']?>" name = "team[team_id]" />
        ...
        <div class="col-md-4">
            ...
            <?php
            if(!empty($form_data['team']['team_image'])){
            ?>
            	<img alt="" src="<?= g('db.admin.bucketimg').'team/'?><?=$form_data['team']['team_image']?>">
            <?php
            }
            else{
            ?>
...
$max_images = 4;
    ...
    {?>
         ...
         <?php if ($form_data['team']['team_is_image'.$i]  == 1): ?>
        ...
        <div class="col-md-4">
            ...
            <?php
            if(!empty($form_data['team']['team_image'.$i])){
            ?>
                <img alt="" src="<?= g('db.admin.bucketimg').'team/'?><?=$form_data['team']['team_image']?>">
            <?php
            }
...
<script>
	$(document).ready(function() {
		...
		$("#saveImageCms").click(function(){
	        ...
	        var url = "<?=$config['base_url']?>admin/team/upload_images/";
