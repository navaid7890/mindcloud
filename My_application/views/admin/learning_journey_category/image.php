...
<form class="cmxform horizontal-form tasi-form" id="uploadCmsimage" method="POST" action="<?= $config['base_url'] ?>admin/learning_journey_category/upload_images">
    <div class="form-body">
        ...
        <input type="hidden" value="<?= $form_data['learning_journey_category']['learning_journey_category_id'] ?>" name="learning_journey_category[learning_journey_category_id]" />
        ...
        <div class="row item_set">
                    ...
                    <div class="">
                        <div data-provides="uploadfile" class="uploadfile uploadfile-new">
                            <div style="max-width: 200px; max-height: 100%;" class="uploadfile-new thumbnail">
                                <?php
                                if (!empty($form_data['learning_journey_category']['learning_journey_category_image'])) {
                                ?>
                                    <img alt="" src="<?= g('db.admin.bucketimg') ?><?= $form_data['learning_journey_category']['learning_journey_category_image'] ?>">
                                <?php
                                } else {
                                ?>
            ...
            for ($i; $i <= $max_images; $i++) { ?>
                ...
                <?php if ($form_data['learning_journey_category']['learning_journey_category_is_image' . $i]  == 1) : ?>
                            ...
                            <div class="">
                                <div data-provides="uploadfile" class="uploadfile uploadfile-new">
                                    <div style="max-width: 200px; max-height: 100%;" class="uploadfile-new thumbnail">
                                        ...
                                        <?php
                                        if (!empty($form_data['learning_journey_category']['learning_journey_category_image' . $i])) {
                                        ?>
                                            <img alt="" src="<?= g('db.admin.bucketimg') ?><?= $form_data['learning_journey_category']['learning_journey_category_image'] ?>">
                                        <?php
                                        } else {
...
<script>
    $(document).ready(function() {
        ...
        $("#saveImageCms").click(function() {
            ...
            var url = "<?= $config['base_url'] ?>admin/learning_journey_category/upload_images/";
