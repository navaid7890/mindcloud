...
<form class="cmxform horizontal-form tasi-form" 
	id="uploadCmsimage" 
	method="POST" 
	action="<?=$config['base_url']?>admin/expert/upload_image" 
>
		<div class="form-body">
        	...
        	<input type = "hidden" value="<?=$form_data['expert']['expert_id']?>" name = "expert[expert_id]" />
        ...
        <div class="col-md-4">
            ...
            ?>
            	<img alt="" src="<?=g('base_url')?>assets/uploads/expert/<?=$form_data['expert']['expert_image']?>">
            <?php
            }
            else{
            ?>
...
$max_images = 4;
    ...
    {?>
         ...
         <?php if ($form_data['expert']['expert_is_image'.$i]  == 1): ?>
        ...
        <div class="col-md-4">
            ...
            <?php
            if(!empty($form_data['expert']['expert_image'.$i])){
            ?>
                <img alt="" src="<?=g('base_url')?>assets/uploads/expert/<?=$form_data['expert']['expert_image'.$i]?>">
            <?php
            }
            else{
            ?>
...
<script>
	$(document).ready(function() {
		...
		$("#saveImageCms").click(function(){
	        ...
	        var url = "<?=$config['base_url']?>admin/expert/upload_images/";
