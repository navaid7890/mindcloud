...
class Course extends MY_Controller {
     ...
     public function detail($slug =' ')
    {
      ...
      //TAB TITLE
        ...
        $data['pop_course'] = $this->model_course->find_all_active($param);
        $data['profession'] = $this->model_profession->find_all_active();
        
          //COURSE PROFESSION
        $param = array();
        $param['fields'] = "profession_name";
        $course_professions =  $this->model_course->get_course_profession($course['course_id'],$param);
        $data['cp'] = $course_professions;
    
      //COURSE LECTUIRES
        $lectures =  $this->model_course->get_lectures($course['course_id']);
        $data['lectures'] = $lectures;
        // debug($data['course_professions']);
        //COMMENT
        $data['user_data'] = $this->model_user->find_by_pk($this->userid);
        $comments = $this->model_comment->get_comments($course['course_id']);

        $average_rating = $this->model_comment->rating($course['course_id']);
      
        $data['comment_info'] = array(
            "proname" => $course['course_name'],
            "pro_id" => $course['course_id'],
...
public function ajax_saveOrder()
    {
      ...
      if ($type == "course") 
      {
        ...
        $course = $this->model_course->find_by_pk_active($id,false,$only);
        ...
        $vars['order_amount'] = $course['course_price'];
     ...
     if ($type == "package") {   
        ...
        if (isset($package_courses) && array_filled($package_courses)) {
          foreach ($package_courses as $key => $value) {
                ...
                $shop_item['item_price'] = $value['course_price'];
      ...
      {   
                $shop_item['item_order_id'] = $order_id;
                $shop_item['item_product_id'] = $course['course_id']; 
                $shop_item['item_product_name'] = $course['course_name'];
                $shop_item['item_product_img'] = get_image($course['course_image'],$course['course_image_path']);
                $shop_item['item_qty'] = 1;
        ...
        if(isset($_GET['item-id']) && $_GET['token'] = md5($_GET['item-id']))
        {
          ...
          {    
            ...
            if ($ptype == 'package-payment') { 
              ...
              $package_courses = $this->model_course->get_package_courses($order_data['order_product_id']);
              $data['package_courses'] = $package_courses;
            }else{
                      //COURSES
              $data['item_code'] = $order_data['course_identity'];
              $type = 'course';
              $data['type'] = $type;
            }
        ...
        // Paypal Payment Option
          $this->load_view('payment-theme',$data);
            ...
            public function course_package($slug)
            {
              ...
              $packages = $this->model_course->get_packages($course['course_id'],$paramp);


              $parmac['fields'] = "course_id,course_name";
              if (isset($packages) && array_filled($packages)) {
                foreach ($packages as $key => $value) {

                  $courses = $this->model_course->get_package_courses($value['package_id'],$parmac);
          ...
          public function lectures()
            {
              ...
              $course = $this->model_course->find_by_pk($lecture['lecture_course_id']);
